...
def get_existing_league(message):
    existing_league = MONGO.servers.find_one({"server": str(message.guild.id)})
    MONGO_CONN.close()
...
def get_existing_player(first_name, last_name, team_abbreviation):
    existing_player = MONGO.players.find_one(
        {
            "name": re.compile(f"{first_name} {last_name}", re.IGNORECASE),
            "team": re.compile(team_abbreviation, re.IGNORECASE),
        }
    )
    MONGO_CONN.close()
...
def get_current_week():
    ...
    nfl_state = requests.get("https://api.sleeper.app/v1/state/nfl")
    nfl_date_list = nfl_state.json()
    starting_date = nfl_date_list["season_start_date"].split("-")
    starting_week = pendulum.datetime(
        int(starting_date[0]),
        int(starting_date[1]),
        int(starting_date[2]),
        tz="America/New_York",
    ...
    if (
        starting_week.is_future()
        or nfl_date_list["season_type"] == "pre"
        or nfl_date_list["season_type"] == "post"
        or nfl_date_list["season_type"] == "off"
    ):
        future = True
        week = 1
    else:
